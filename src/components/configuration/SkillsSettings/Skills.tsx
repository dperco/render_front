...
export default function Jobs() {
  /* Estado de datos */
  const [skills, setSkills] = useState<Job[]>([]);
  const [skillDelete, setSkillDelete] = useState<any>(undefined);
  ...
  const handleClickDelete = (job: any) => {
    setSkillDelete(job);
    setModalMessage("Desea eliminar el skill" + job.name + " ?");
  ...
  async function handleDelete() {
    ...
    setModalMessage(
      "Desea eliminar el skill" + skillDelete.name + " ?"
    );
    setModalVariant("warning");
    // return;
    const res = await deleteSkill(skillDelete.id);
    if (res.message) {
      setModalMessage(res.message || "Skill eliminada");
